...
import cv2
import math
from util.cv_util import find_mutual_matached_keypoints,estimateAffineRansac
...
def correct_keypoints(source_points,warped_points,L_pck,alpha=0.1):
    ...
    num_of_correct_points = torch.sum(correct_points)
    num_of_points = correct_points.numel()
    temp = torch.sum(torch_sum)
    reprojection_error = math.sqrt(temp)
...
for i, batch in enumerate(dataloader):
    ...
    # if do_ransac:
    #     source_points_np, target_points_np = source_points.data.numpy()[0].reshape((-1,2)), target_points.data.numpy()[0].reshape((-1,2))
    ...
    if do_aff:
        ...
        if do_ransac:
            ...
            print(keypoints_A)
            print(keypoints_B)
            tensor_size = np.asarray(correlationAB.shape)
            ...
            M = estimateAffineRansac(keyptsA_tensor_var.data.numpy().reshape((-1,2)),keyptsB_tensor_var.data.numpy().reshape((-1,2)))
            ransac_success = M is not None
            if ransac_success:
                M_var = Variable(torch.FloatTensor(M.flatten()))
                # do affine only
                ...
                warped_points_ransac = PointsToPixelCoords(warped_points_ransac_norm, source_im_size)
    if do_tps:
        theta_tps,correlation_tps,_ =model_tps(batch)
    ...
    if do_aff and do_tps:
        ...
        theta_aff_tps,correlation_aff_tps,_=model_tps({'source_image': warped_image_aff, 'target_image': batch['target_image']})
